...
### 2. 소재 선택 — `TOPICS` 풀에서 날짜순 순환 (threads_post.py)
월~토 매일, 소재 풀을 날짜 기준으로 순환 선택(일요일 휴무). **블로그/마케팅 6종 + 유튜브 6종**.

- 블로그·마케팅: 블로그 주의사항 / 플레이스·블로그 상위노출 / 보건소 신고 대응 / 사전심의 실무 / 문의→예약 전환 / 마케팅 꿀팁
- 유튜브: 채널 시작 전 정할 것 / 영상 주제 뽑기 / 직접 촬영 효율 / 썸네일·제목 / 유튜브 의료광고법 주의 / 시청자→내원 전환

> 화자 = **병원·전문직을 돕는 마케팅 전문가(대행사)**. 원장님께 유용한 실전 팁·대응법을 나눔.
> 소재 추가는 `TOPICS` 리스트에 항목만 추가하면 됨.
...
| `THREADS_USER_ID` | Threads 사용자 ID (0ra_marketing) = `28173366672261513` | ✅ 등록됨 |
| `THREADS_ACCESS_TOKEN` | Threads 장기 액세스 토큰 (0ra_marketing) | ✅ 등록됨 |

> ✅ **실제 게시 검증 완료** (2026-07-15): 수동 실행으로 main + 이어쓰기 2 + 댓글이 `0ra_marketing`에 정상 게시됨.
